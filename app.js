...
function liveSyncViewImpacted(view,resources){
 const map={
  ...
  talleres:['TALLERES','TALLER_EVALUACIONES','MANTENCIONES','ORDENES_TRABAJO'],historial:['MANTENCIONES','ORDENES_TRABAJO','FALLAS'],
...
function kpiGlyph(label=''){
 ...
 if(x.includes('salud'))return '❤️';
 if(x.includes('riesgo'))return '⚠️';
 if(x.includes('crít')||x.includes('crit'))return '🚨';
 if(x.includes('más fallas'))return '🚨';
 if(x.includes('menos fallas'))return '✅';
 if(x.includes('reincid'))return '🔁';
 if(x.includes('falla'))return '⚠️';
 if(x.includes('mant'))return '🔧';
 if(x.includes('observ'))return '👁️';
 if(x.includes('aprob'))return '✅';
 if(x.includes('no apt'))return '⛔';
 if(x.includes('check')||x.includes('inspecc'))return '📋';
 if(x.includes('calidad')||x.includes('nota'))return '⭐';
 if(x.includes('evaluación')||x.includes('evaluacion'))return '👍';
 if(x.includes('taller'))return '🏭';
 if(x.includes('conductor'))return '👤';
 if(x.includes('usuario')||x.includes('gestión')||x.includes('gestion')||x.includes('perfil'))return '👥';
 if(x.includes('veh'))return '🚙';
 if(x.includes('combust')||x.includes('litro'))return '⛽';
 if(x.includes('document'))return '📄';
 if(x.includes('notific'))return '🔔';
 if(x.includes('veloc'))return '🏁';
 if(x.includes('costo')||x.includes('gasto')||x.includes('presupuesto')||x.includes('ejecutado')||x.includes('disponible')||x.includes('comprometido')||x.includes('fondo'))return '💰';
 if(x.includes('proyección')||x.includes('proyeccion')||x.includes('desviación')||x.includes('desviacion'))return '📈';
 if(x.includes('km'))return '🛣️';
 if(x.includes('día')||x.includes('dia')||x.includes('vencid'))return '📅';
 if(x.includes('pendient'))return '⏳';
 if(x.includes('prioridad'))return '🔺';
 if(x.includes('activo')||x.includes('operativ')||x.includes('vigente'))return '✅';
 if(x.includes('anomal'))return '🔎';
 if(x.includes('predic'))return '🧠';
 if(x.includes('registros')||x.includes('total'))return '📊';
 return '📊';
}
...
function faultClosedCount(x){return Number(x.fallas_cerradas!=null?x.fallas_cerradas:Math.max(0,Number(x.total_fallas||0)-Number(x.fallas_abiertas||0)))}
function renderFaultVehicleChart(rows=[]){
 const all=[...(rows||[])].map(x=>({...x,total_fallas:Number(x.total_fallas||0),fallas_abiertas:Number(x.fallas_abiertas||0),fallas_cerradas:faultClosedCount(x)}));
 const summary=$('faultVehicleSummary'),chart=$('faultVehicleChart');if(!summary||!chart)return;
 if(!all.length){summary.innerHTML='';chart.innerHTML='<div class="notification-empty">Aún no hay vehículos disponibles para comparar fallas.</div>';return}
 const byTotal=[...all].sort((a,b)=>b.total_fallas-a.total_fallas||String(a.patente||'').localeCompare(String(b.patente||''),'es'));
 const byLeast=[...all].sort((a,b)=>a.total_fallas-b.total_fallas||String(a.patente||'').localeCompare(String(b.patente||''),'es'));
 const byOpen=[...all].sort((a,b)=>b.fallas_abiertas-a.fallas_abiertas||b.total_fallas-a.total_fallas);
...
const ADV_FILTERS={
 ...
 talleres:{state:['estado'],category:['especialidad','evaluacion','categoria'],criticality:['criticidad','prioridad'],date:['actualizado_en','creado_en']},
...
function renderCheckinHistory(){const rows=filteredCheckinHistory(),approved=rows.filter(x=>String(x.estado||'').toUpperCase()==='APROBADO'||String(x.aprobacion_directa||'').toUpperCase()==='SI'||String(x.aprobacion_automatica||'').toUpperCase()==='SI').length,observed=rows.filter(x=>/OBS/.test(String(x.resultado_tecnico||x.estado||'').toUpperCase())||String(x.requiere_decision||'').toUpperCase()==='SI').length,noApto=rows.filter(x=>String(x.resultado_tecnico||'').toUpperCase().includes('NO APTO')).length;$('checkinHistoryKpis').innerHTML=ringKpi('Inspecciones',rows.length,Math.min(100,rows.length*2),'blue','Historial filtrado')+ringKpi('Aprobados',approved,rows.length?approved/rows.length*100:0,'green','Operacionales')+ringKpi('Observados',observed,rows.length?observed/rows.length*100:0,observed?'amber':'green','Con observación / decisión')+ringKpi('No aptos',noApto,rows.length?noApto/rows.length*100:0,noApto?'red':'green','Requieren atención');$('checkinHistoryRows').innerHTML=rows.length?rows.map(checkinHistoryCard).join(''):'<div class="notification-empty">No hay Checklist para los filtros seleccionados.</div>'}
...
const WORKSHOP_EVAL_CRITERIA=[
 {key:'especializacion_equipamiento',icon:'🛠️',label:'Especialización y equipamiento',detail:'Diagnóstico computarizado, elevadores, herramientas y técnicos especializados.'},
 {key:'precision_diagnostico',icon:'🎯',label:'Precisión en los diagnósticos',detail:'Identifica la causa raíz correctamente y evita cambiar piezas innecesarias.'},
 {key:'transparencia_repuestos',icon:'⚙️',label:'Transparencia y calidad de repuestos',detail:'Original/OEM/alternativo, procedencia y garantía informadas.'},
 {key:'cumplimiento_horarios',icon:'⏱️',label:'Cumplimiento de horarios',detail:'Entrega el vehículo en el plazo comprometido.'},
 {key:'proximidad_base',icon:'📍',label:'Proximidad a la base',detail:'Facilidad logística por cercanía al punto de operaciones.'},
 {key:'transparencia_presupuesto',icon:'💰',label:'Transparencia y presupuestos',detail:'Presupuesto escrito, detallado y autorización ante imprevistos.'},
 {key:'garantia_servicio',icon:'🛡️',label:'Garantía del servicio',detail:'Garantía formal de reparación y piezas instaladas.'},
 {key:'reputacion_referencias',icon:'⭐',label:'Reputación y referencias',detail:'Opiniones, atención y resolución de reclamos.'},
 {key:'orden_infraestructura',icon:'🧹',label:'Orden, limpieza e infraestructura',detail:'Organización, zonas de trabajo y cuidado del vehículo.'},
 {key:'cumplimiento_legal',icon:'📋',label:'Cumplimiento legal y normativo',detail:'Formalización, boleta/factura y manejo de residuos peligrosos.'}
];
function workshopEvalLabel(v){const x=String(v||'A_VECES').toUpperCase().replace(/\s+/g,'_');return x==='SI'?'Sí':x==='NO'?'No':'A veces'}
function workshopStars(n){const v=Math.max(1,Math.min(5,Math.round(Number(n||3))));return '★'.repeat(v)+'☆'.repeat(5-v)}
function workshopEvaluationRows(tallerId){return (S.workshopEvaluations||[]).filter(x=>String(x.taller_id)===String(tallerId)&&String(x.eliminado||'NO').toUpperCase()!=='SI').sort((a,b)=>new Date(b.fecha_hora||b.creado_en||0)-new Date(a.fecha_hora||a.creado_en||0))}
function workshopEvalAverage(row){if(Number(row?.nota_promedio)>0)return Number(row.nota_promedio);const vals=WORKSHOP_EVAL_CRITERIA.map(c=>Number(row?.[c.key]||0)).filter(v=>v>=1&&v<=5);return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0}
function workshopSummary(taller){const evs=workshopEvaluationRows(taller.id),avg=evs.length?evs.reduce((a,e)=>a+workshopEvalAverage(e),0)/evs.length:Number(taller.nota_calidad_promedio||taller.nota_calidad||Math.round(Number(taller.calidad_porcentaje||60)/20)||3),latest=evs[0]||null;return{evs,avg,latest,total:evs.length,general:latest?.evaluacion_general||taller.evaluacion||'A_VECES'}}
async function loadWorkshops(){const [tw,ev]=await Promise.all([api('listar',{recurso:'TALLERES',limit:500}),api('listar',{recurso:'TALLER_EVALUACIONES',limit:500}).catch(()=>({rows:[]}))]);S.talleres=tw.rows||[];S.workshopEvaluations=ev.rows||[];S.rows.TALLERES=S.talleres;S.rows.TALLER_EVALUACIONES=S.workshopEvaluations;populateAdvancedFilter('talleres',S.talleres);renderWorkshops()}
function workshopCriterionAverages(evs){const out={};for(const c of WORKSHOP_EVAL_CRITERIA){const vals=evs.map(e=>Number(e[c.key]||0)).filter(v=>v>=1&&v<=5);out[c.key]=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0}return out}
function renderWorkshopQualityChart(evs){const box=$('workshopQualityChart');if(!box)return;if(!evs.length){box.innerHTML='<div class="notification-empty">Aún no hay evaluaciones formales de talleres.</div>';return}const avg=workshopCriterionAverages(evs);box.innerHTML=`<div class="workshop-quality-head"><div><span class="module-eyebrow">CALIDAD DE LA RED</span><h4>Promedio por criterio</h4></div><strong>${evs.length} evaluación(es)</strong></div><div class="workshop-quality-bars">${WORKSHOP_EVAL_CRITERIA.map(c=>{const v=avg[c.key]||0;return `<div class="workshop-quality-row"><span class="workshop-quality-label"><b>${c.icon}</b><span><strong>${esc(c.label)}</strong><small>${esc(c.detail)}</small></span></span><div class="workshop-quality-meter"><i style="width:${Math.max(0,Math.min(100,v/5*100))}%"></i></div><em>${v?`${v.toFixed(1)}/5`:'—'}</em></div>`}).join('')}</div>`}
function renderWorkshops(){const rows=advancedFilteredRows('talleres',S.talleres||[]),allEvs=(S.workshopEvaluations||[]).filter(x=>String(x.eliminado||'NO').toUpperCase()!=='SI'&&rows.some(t=>String(t.id)===String(x.taller_id))),active=rows.filter(x=>String(x.estado||'').toUpperCase()==='ACTIVO').length,avgNote=allEvs.length?allEvs.reduce((a,x)=>a+workshopEvalAverage(x),0)/allEvs.length:(rows.length?rows.reduce((a,x)=>a+Number(x.nota_calidad_promedio||x.nota_calidad||3),0)/rows.length:0),yes=allEvs.filter(x=>String(x.evaluacion_general||'').toUpperCase()==='SI').length,sometimes=allEvs.filter(x=>String(x.evaluacion_general||'').toUpperCase()==='A_VECES').length,no=allEvs.filter(x=>String(x.evaluacion_general||'').toUpperCase()==='NO').length;$('workshopKpis').innerHTML=ringKpi('🏭 Talleres',rows.length,Math.min(100,rows.length*10),'blue','Red técnica')+ringKpi('✅ Activos',active,rows.length?active/rows.length*100:0,'green','Disponibles')+ringKpi('⭐ Calidad promedio',allEvs.length?`${avgNote.toFixed(1)}/5`:'—',avgNote/5*100,avgNote<2.5?'red':avgNote<4?'amber':'green',`${allEvs.length} evaluación(es)`)+ringKpi('👍 Sí',yes,allEvs.length?yes/allEvs.length*100:0,'green','Recomendación')+ringKpi('🤔 A veces',sometimes,allEvs.length?sometimes/allEvs.length*100:0,'amber','Recomendación')+ringKpi('👎 No',no,allEvs.length?no/allEvs.length*100:0,no?'red':'green','Recomendación');renderWorkshopQualityChart(allEvs);$('talleresRows').innerHTML=rows.length?rows.map(x=>{const sm=workshopSummary(x),note=sm.avg||3;return `<article class="workshop-card"><div class="workshop-head"><span>🏭</span><div><h4>${esc(x.nombre)}</h4><p>${esc(x.especialidad||'Taller general')}</p></div><span class="badge ${String(x.estado).toUpperCase()==='ACTIVO'?'ok':'warn'}">${esc(x.estado||'ACTIVO')}</span></div><div class="workshop-info"><span class="workshop-address"><small>DIRECCIÓN</small><strong>${esc(x.direccion_normalizada||x.direccion||'Sin dirección')}</strong>${x.comuna||x.ciudad?`<em>${esc([x.comuna,x.ciudad].filter(Boolean).join(' · '))}</em>`:''}</span><span><small>TELÉFONO</small><strong>${esc(x.telefono||'—')}</strong></span><span><small>CONTACTO</small><strong>${esc(x.contacto||x.correo||'—')}</strong></span><span><small>RECOMENDACIÓN</small><strong>${esc(workshopEvalLabel(sm.general))}</strong></span><span><small>CALIDAD HISTÓRICA</small><strong class="workshop-stars">${workshopStars(note)} <em>${note.toFixed(1)}/5 · ${sm.total} eval.</em></strong></span></div><div class="card-actions"><button class="mini detail" data-workshop-map="${esc(x.id)}">📍 Ver ubicación</button>${(permissionAllowed('TALLERES','EDITAR')||permissionAllowed('TALLERES','CREAR'))?`<button class="mini edit" data-workshop-evaluate="${esc(x.id)}">⭐ Evaluar taller</button>`:''}<button class="mini detail" data-workshop-eval-history="${esc(x.id)}">↺ Historial evaluación</button></div>${adminActions('taller',x.id)}</article>`}).join(''):'<div class="notification-empty">No hay talleres para los filtros seleccionados.</div>'}
...
function ensureWorkshopEvaluationModal(){let modal=$('workshopEvaluationModal');if(modal)return modal;document.body.insertAdjacentHTML('beforeend',`<div id="workshopEvaluationModal" class="modal hidden"><div class="modal-card workshop-eval-modal"><div class="modal-head"><div><span class="module-eyebrow">EVALUACIÓN FORMAL</span><h3 id="workshopEvaluationTitle">Evaluar taller</h3></div><button id="workshopEvaluationClose" class="ghost" type="button">✕</button></div><div id="workshopEvaluationBody"></div><div class="modal-actions"><button id="workshopEvaluationCancel" class="ghost" type="button">Cancelar</button><button id="workshopEvaluationSave" class="primary" type="button">Guardar evaluación</button></div></div></div>`);modal=$('workshopEvaluationModal');$('workshopEvaluationClose').onclick=$('workshopEvaluationCancel').onclick=()=>modal.classList.add('hidden');$('workshopEvaluationSave').onclick=saveWorkshopEvaluation;return modal}
async function openWorkshopEvaluation(id){const t=S.talleres.find(x=>String(x.id)===String(id));if(!t)return;await ensureCatalogs();if(!(S.rows.MANTENCIONES||[]).length){try{const j=await api('listar',{recurso:'MANTENCIONES',limit:500});S.rows.MANTENCIONES=j.rows||[]}catch{}}if(!S.orders.length){try{const j=await api('listar',{recurso:'ORDENES_TRABAJO',limit:500});S.orders=j.rows||[]}catch{}}const modal=ensureWorkshopEvaluationModal();modal.dataset.tallerId=id;$('workshopEvaluationTitle').textContent=`Evaluar · ${t.nombre}`;$('workshopEvaluationBody').innerHTML=`<p class="workshop-eval-intro">Califica cada criterio entre <b>1 = Muy deficiente</b> y <b>5 = Excelente</b>. La nota final se calcula automáticamente.</p><div class="workshop-eval-links"><label>Vehículo relacionado<select id="workshopEvalVehicle"><option value="">Sin vehículo específico</option>${S.vehicles.map(v=>`<option value="${esc(v.id)}">${esc(v.patente)} · ${esc([v.marca,v.modelo].filter(Boolean).join(' '))}</option>`).join('')}</select></label><label>Mantención relacionada<select id="workshopEvalMaintenance"><option value="">Sin mantención específica</option>${(S.rows.MANTENCIONES||[]).filter(m=>!m.taller_id||String(m.taller_id)===String(id)).map(m=>`<option value="${esc(m.id)}">${esc(m.descripcion||m.id)}</option>`).join('')}</select></label><label>OT relacionada<select id="workshopEvalOrder"><option value="">Sin OT específica</option>${(S.orders||[]).filter(o=>!o.taller_id||String(o.taller_id)===String(id)).map(o=>`<option value="${esc(o.id)}">${esc(o.titulo||o.id)}</option>`).join('')}</select></label></div><div class="workshop-criteria-grid">${WORKSHOP_EVAL_CRITERIA.map(c=>`<label class="workshop-score-row"><span><b>${c.icon} ${esc(c.label)}</b><small>${esc(c.detail)}</small></span><select data-workshop-score="${c.key}"><option value="1">1 · Muy deficiente</option><option value="2">2 · Deficiente</option><option value="3" selected>3 · Aceptable</option><option value="4">4 · Bueno</option><option value="5">5 · Excelente</option></select></label>`).join('')}</div><label>¿Recomendarías este taller?<select id="workshopEvalGeneral"><option value="SI">Sí</option><option value="A_VECES" selected>A veces</option><option value="NO">No</option></select></label><label>Comentario / observación<textarea id="workshopEvalComment" rows="4" placeholder="Fortalezas, problemas, reclamos, garantía, repuestos, cumplimiento de plazo…"></textarea></label><div id="workshopEvalPreview" class="workshop-eval-preview">⭐ Nota estimada: <strong>3.0/5</strong></div>`;modal.querySelectorAll('[data-workshop-score]').forEach(x=>x.onchange=updateWorkshopEvaluationPreview);updateWorkshopEvaluationPreview();modal.classList.remove('hidden')}
function updateWorkshopEvaluationPreview(){const vals=[...document.querySelectorAll('#workshopEvaluationModal [data-workshop-score]')].map(x=>Number(x.value||3)),avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;const out=$('workshopEvalPreview');if(out)out.innerHTML=`⭐ Nota estimada: <strong>${avg.toFixed(1)}/5</strong>`}
async function saveWorkshopEvaluation(){const modal=$('workshopEvaluationModal'),b=$('workshopEvaluationSave'),tallerId=modal?.dataset.tallerId;if(!tallerId)return;loading(b,true);try{const row={id:'TEV-'+crypto.randomUUID().toUpperCase(),taller_id:tallerId,vehiculo_id:$('workshopEvalVehicle')?.value||null,mantencion_id:$('workshopEvalMaintenance')?.value||null,orden_trabajo_id:$('workshopEvalOrder')?.value||null,evaluacion_general:$('workshopEvalGeneral')?.value||'A_VECES',comentario:String($('workshopEvalComment')?.value||'').trim()||null,fecha_hora:new Date().toISOString()};document.querySelectorAll('#workshopEvaluationModal [data-workshop-score]').forEach(x=>row[x.dataset.workshopScore]=Number(x.value||3));await api('guardar',{recurso:'TALLER_EVALUACIONES',row});modal.classList.add('hidden');toast('Evaluación de taller guardada');await loadWorkshops()}catch(e){toast('Evaluación de taller: '+friendlyError(e.message),true)}finally{loading(b,false)}}
function ensureWorkshopEvaluationHistoryModal(){let modal=$('workshopEvaluationHistoryModal');if(modal)return modal;document.body.insertAdjacentHTML('beforeend',`<div id="workshopEvaluationHistoryModal" class="modal hidden"><div class="modal-card workshop-eval-history-modal"><div class="modal-head"><div><span class="module-eyebrow">TRAZABILIDAD DE CALIDAD</span><h3 id="workshopEvaluationHistoryTitle">Historial de evaluación</h3></div><button id="workshopEvaluationHistoryClose" class="ghost" type="button">✕</button></div><div id="workshopEvaluationHistoryBody"></div><div class="modal-actions"><button id="workshopEvaluationHistoryDone" class="primary" type="button">Cerrar</button></div></div></div>`);modal=$('workshopEvaluationHistoryModal');$('workshopEvaluationHistoryClose').onclick=$('workshopEvaluationHistoryDone').onclick=()=>modal.classList.add('hidden');return modal}
function openWorkshopEvaluationHistory(id){const t=S.talleres.find(x=>String(x.id)===String(id));if(!t)return;const evs=workshopEvaluationRows(id),sm=workshopSummary(t),modal=ensureWorkshopEvaluationHistoryModal();$('workshopEvaluationHistoryTitle').textContent=`Historial de evaluación · ${t.nombre}`;$('workshopEvaluationHistoryBody').innerHTML=`<div class="workshop-history-summary"><strong>⭐ ${sm.avg.toFixed(1)}/5</strong><span>${sm.total} evaluación(es)</span><span>Recomendación actual: ${esc(workshopEvalLabel(sm.general))}</span></div>${evs.length?`<div class="workshop-eval-timeline">${evs.map(e=>`<article><div class="timeline-dot"></div><div><div class="workshop-eval-history-head"><strong>${new Date(e.fecha_hora||e.creado_en).toLocaleString('es-CL')}</strong><span class="badge">${esc(workshopEvalLabel(e.evaluacion_general))}</span><span class="workshop-stars">${workshopStars(workshopEvalAverage(e))} ${workshopEvalAverage(e).toFixed(1)}/5</span></div><small>Evaluado por: ${esc(e.evaluado_por_nombre||'Usuario autorizado')}${e.vehiculo_patente?' · 🚙 '+esc(e.vehiculo_patente):''}${e.orden_trabajo_id?' · OT asociada':''}</small><div class="workshop-history-criteria">${WORKSHOP_EVAL_CRITERIA.map(c=>`<span><b>${c.icon}</b>${esc(c.label)} <strong>${Number(e[c.key]||0)}/5</strong></span>`).join('')}</div>${e.comentario?`<p>${esc(e.comentario)}</p>`:''}</div></article>`).join('')}</div>`:'<div class="notification-empty">Este taller todavía no tiene evaluaciones formales.</div>'}`;modal.classList.remove('hidden')}
...
function reportNormalizeRow(x){
 ...
 if(out.evaluado_por){out.evaluado_por_nombre=out.evaluado_por_nombre||userName(out.evaluado_por);delete out.evaluado_por}
...
function reportKpiHtml(type,rows){
 ...
 if(type==='TALLER_EVALUACIONES'){const quality=total?rows.reduce((a,x)=>a+Number(x.nota_promedio||workshopEvalAverage(x)||0),0)/total:0,yes=rows.filter(x=>up(x.evaluacion_general)==='SI').length,sometimes=rows.filter(x=>up(x.evaluacion_general)==='A_VECES').length,no=rows.filter(x=>up(x.evaluacion_general)==='NO').length;return ringKpi('⭐ Evaluaciones',total,Math.min(100,total*5),'blue','Historial')+ringKpi('⭐ Calidad',quality?quality.toFixed(1)+'/5':'—',quality/5*100,quality<2.5?'red':quality<4?'amber':'green','10 criterios')+ringKpi('👍 Sí',yes,pct(yes),'green','Recomendación')+ringKpi('🤔 A veces',sometimes,pct(sometimes),'amber','Recomendación')+ringKpi('👎 No',no,pct(no),no?'red':'green','Recomendación');}
...
function budgetAuditValue(v,initial=false){if(v===null||v===undefined)return initial?'Sin valor inicial':'Sin valor';if(typeof v==='string'&&['','null','undefined'].includes(v.trim().toLowerCase()))return initial?'Sin valor inicial':'Sin valor';if(typeof v==='object'){try{return JSON.stringify(v)}catch(e){return String(v)}}return String(v)}
...
function renderBudget(s){
 ...
 $('budgetAudit').innerHTML=(s.auditoria||[]).length?s.auditoria.slice(0,30).map(a=>{const before=budgetAuditValue(a.valor_anterior,true),after=budgetAuditValue(a.valor_nuevo,false),initial=before==='Sin valor inicial';return `<article class="budget-audit-card"><small>${a.creado_en?new Date(a.creado_en).toLocaleString('es-CL'):'—'}<br>${esc(a.rol||a.rol_usuario||'Gestión')}</small><div><strong>${esc(a.accion||'EVENTO')} · ${esc(a.campo||a.entidad_tipo||'Presupuesto')}</strong><div class="budget-audit-change">${initial?`Valor inicial: ${esc(after)}`:`Antes: ${esc(before)} → Después: ${esc(after)}`}</div><small>${esc(a.motivo||'Sin motivo registrado')}${a.desviacion_previa!=null?` · Desviación previa: ${budgetMoney(a.desviacion_previa)}`:''}</small></div></article>`}).join(''):'<div class="budget-no-data">Sin movimientos de auditoría.</div>';
}
...
async function openDashboardDetail(kind){
 ...
 try{
  ...
  else if(kind==='SALUD_FLOTA'||kind==='RIESGO_FLOTA'){title=kind==='SALUD_FLOTA'?'Salud de flota':'Riesgo operacional por vehículo';const [j,dash]=await Promise.all([api('listar',{recurso:'SALUD_VEHICULO',limit:500}),api('dashboard',{},true)]);const faults=new Map((dash.fallasVehiculo||[]).map(x=>[String(x.vehiculo_id),x]));rows=(j.rows||[]).sort((a,b)=>kind==='RIESGO_FLOTA'?Number(b.riesgo_porcentaje||0)-Number(a.riesgo_porcentaje||0):Number(a.salud_porcentaje||0)-Number(b.salud_porcentaje||0));$('modalBody').innerHTML=`<div class="risk-vehicle-list">${rows.map(x=>{const f=faults.get(String(x.vehiculo_id))||{},health=Math.max(0,Math.min(100,Number(x.salud_porcentaje||0))),risk=Math.max(0,Math.min(100,Number(x.riesgo_porcentaje||0))),closed=f.fallas_cerradas!=null?Number(f.fallas_cerradas):Math.max(0,Number(f.total_fallas||0)-Number(f.fallas_abiertas||0));return `<article class="risk-vehicle-card clickable" data-life-vehicle="${esc(x.vehiculo_id)}"><div class="risk-vehicle-title"><span>${kind==='RIESGO_FLOTA'?'⚠️':'❤️'}</span><div><strong>${esc(x.vehiculo_patente||vehicleName(x.vehiculo_id))}</strong><small>${esc(x.tendencia||'SIN TENDENCIA')}</small></div></div><div class="risk-meter-row"><span>Salud ${health.toFixed(0)}%</span><i><b class="health" style="width:${health}%"></b></i></div><div class="risk-meter-row"><span>Riesgo ${risk.toFixed(0)}%</span><i><b class="risk" style="width:${risk}%"></b></i></div><div class="risk-fault-counts"><span>⚠️ Abiertas <b>${Number(f.fallas_abiertas||0)}</b></span><span>✔️ Cerradas <b>${closed}</b></span><span>🚨 Críticas <b>${Number(f.fallas_criticas||0)}</b></span></div>${x.explicacion?`<p>${esc(String(x.explicacion).slice(0,220))}</p>`:''}</article>`}).join('')}</div>`;}
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 document.addEventListener('click',e=>{
   ...
   const workshopEvaluate=e.target.closest('[data-workshop-evaluate]');if(workshopEvaluate){openWorkshopEvaluation(workshopEvaluate.dataset.workshopEvaluate);return;}
