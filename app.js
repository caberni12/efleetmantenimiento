...
function renderFaultVehicleChart(rows=[]){
 const all=[...(rows||[])].map(x=>({...x,total_fallas:Number(x.total_fallas||0)}));
 ...
 const sorted=[...all].sort((a,b)=>Number(b.total_fallas||0)-Number(a.total_fallas||0)||String(a.patente||'').localeCompare(String(b.patente||''),'es'));
 const most=sorted[0],least=[...all].sort((a,b)=>Number(a.total_fallas||0)-Number(b.total_fallas||0)||String(a.patente||'').localeCompare(String(b.patente||''),'es'))[0];
 const driver=x=>x.conductor_nombre||'Sin conductor vigente';
 summary.innerHTML=`<article class="fault-extreme most" data-life-vehicle="${esc(most.vehiculo_id)}"><span>🚨</span><div><small>MÁS FALLAS</small><strong>${esc(most.patente||most.vehiculo_id||'Vehículo')}</strong><p>${Number(most.total_fallas||0)} falla(s) · 👤 ${esc(driver(most))}</p></div></article><article class="fault-extreme least" data-life-vehicle="${esc(least.vehiculo_id)}"><span>✅</span><div><small>MENOS FALLAS</small><strong>${esc(least.patente||least.vehiculo_id||'Vehículo')}</strong><p>${Number(least.total_fallas||0)} falla(s) · 👤 ${esc(driver(least))}</p></div></article>`;
 const max=Math.max(1,...sorted.map(x=>Number(x.total_fallas||0)));const top=sorted.slice(0,10);
 chart.innerHTML=top.map(x=>`<button class="fault-chart-row" type="button" data-life-vehicle="${esc(x.vehiculo_id)}"><span class="fault-chart-label"><b>${esc(x.patente||x.vehiculo_id||'Vehículo')}</b><small>${esc(driver(x))}</small></span><span class="fault-chart-track"><i style="width:${Math.max(3,Math.round(Number(x.total_fallas||0)/max*100))}%"></i></span><strong>${Number(x.total_fallas||0)}</strong></button>`).join('');
}
...
async function openDashboardDetail(kind){
 ...
 try{
  ...
  else if(kind==='SALUD_FLOTA'||kind==='RIESGO_FLOTA'){title=kind==='SALUD_FLOTA'?'Salud de flota':'Riesgo operacional por vehículo';const j=await api('listar',{recurso:'SALUD_VEHICULO',limit:500});rows=(j.rows||[]).sort((a,b)=>kind==='RIESGO_FLOTA'?Number(b.riesgo_porcentaje||0)-Number(a.riesgo_porcentaje||0):Number(a.salud_porcentaje||0)-Number(b.salud_porcentaje||0));$('modalBody').innerHTML=lifeList(title,rows,x=>`<div class="life-row clickable" data-life-vehicle="${esc(x.vehiculo_id)}"><span>${kind==='RIESGO_FLOTA'?'⚠':'♥'}</span><div><strong>${esc(x.vehiculo_patente||vehicleName(x.vehiculo_id))}</strong><small>Salud del vehículo ${Number(x.salud_porcentaje||0).toFixed(0)}% · Riesgo ${Number(x.riesgo_porcentaje||0).toFixed(0)}%${x.tendencia?' · '+esc(x.tendencia):''}</small>${x.explicacion?`<em>${esc(String(x.explicacion).slice(0,160))}</em>`:''}</div></div>`);}
