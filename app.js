...
async function loadSpeedModule(){
 ...
 const vehicleBox=$('speedVehicleLimits'),eventBox=$('speedEvents');if(!vehicleBox||!eventBox)return;
 vehicleBox.innerHTML=S.vehicles.length?S.vehicles.map(v=>`<article class="speed-limit-card"><div><small>PATENTE</small><h4>${esc(v.patente||'Vehículo')}</h4><p>${esc([v.marca,v.modelo].filter(Boolean).join(' '))}</p></div><label>Límite km/h<input type="number" min="10" max="220" step="1" value="${esc(v.limite_velocidad_kmh??'')}" placeholder="Heredado"></label><button class="primary" type="button" data-speed-limit-save="${esc(v.id)}">Guardar límite</button></article>`).join(''):'<div class="notification-empty">No hay vehículos disponibles.</div>';
